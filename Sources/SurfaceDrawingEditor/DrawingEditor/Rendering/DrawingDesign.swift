//
//  DrawingDesign.swift
//  DetectObjectsDemo
//
//  Created by _d3n_o77 on 26.02.2026.
//

import UIKit
import SwiftUI

// MARK: - Design Tokens
enum DrawingDesign {
    static let accentColor = UIColor(red: 1.0, green: 0.72, blue: 0.0, alpha: 1.0)
    static let accentSwiftUI = Color(hex: "#FFBB00")
    static let brushColor = Color(hex: "#BEBEBE")
    static let eraserCursorColor = UIColor(red: 0.2, green: 0.2, blue: 0.2, alpha: 0.7)
    
    static let brushDisplayColor = UIColor(red: 0xEB/255, green: 0x15/255, blue: 0x15/255, alpha: 1)
    static let brushRenderColor = UIColor(red: 190/255, green: 190/255, blue: 190/255, alpha: 1)
}
